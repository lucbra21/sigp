...
def generate_contract_pdf(prescriptor, filename: Optional[str] = None) -> Path:
    ...
    y_clausulas = draw_paragraph(texto_comision, margin_x, y_clausulas, width - 2 * margin_x)
    ...
    y -= 6
    y = draw_paragraph(
        "Ejemplo: Si la comisión establecida es €150, este será el importe final que recibirá EL COLABORADOR, independientemente de su situación fiscal o costos bancarios",
    ...
    clausula_pago = [
        f"Pago Único: Se abonará el 100% de la comisión ({monto_comision} €) por cada alumno referido.",
        "Momento del Pago: La comisión se devengará íntegramente en el momento en que el alumno realice el pago efectivo de la MATRÍCULA.",
        "Independencia de Cuotas: El pago al colaborador no estará sujeto a que el alumno complete el pago de las cuotas mensuales del programa, siempre que la matrícula haya sido abonada.",
    ]
    y = draw_bullets(clausula_pago, margin_x, y, width - 2 * margin_x)

    y -= 15
    y = draw_paragraph(
        "Liquidación: Las comisiones autorizadas tras el pago de matrícula se abonarán en los primeros diez (10) días hábiles del mes siguiente a través del SIGP.",
    ...
    y -= 15
    y = draw_paragraph(
        "Las comisiones autorizadas se abonarán en los primeros diez (10) días hábiles del mes siguiente, conforme a las condisiones establecidas previamente.",
        margin_x, y, width - 2 * margin_x
    )

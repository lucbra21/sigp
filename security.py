...
Assumes naming convention `<module>_<action>` (e.g. `users_create`).
Works with reflected tables `users`, `roles`, `permissions`, `role_permissions`.
"""
...
# Obtain reflected classes (may be None if schema missing)
...
UserRole = None  # automap may create roles relationship automatically

_perm_cache_attr = "_cached_perm_names"

def _permission_names_for_user(user) -> Set[str]:
    """Return set of permission names for the given user object."""
    if not (user and user.is_authenticated and Permission is not None and RolePerm is not None and Role is not None):
        return set()

    # Collect role IDs for the user. Automap usually produces a `roles` relationship.
    role_ids = []
    # 1) Relación many-to-many (iterable) o atributo escalar 'roles'
    ...
    if not role_ids:
        ...
        for cls in Base.classes.values():
            ...
            if {"user_id", "role_id"}.issubset(cols):
                ...
                break
        if assoc:
            role_ids = [row.role_id for row in db.session.query(assoc.role_id)
    ...
    if RolePerm is not None:
        # If RolePerm is an ORM mapped class
        if hasattr(RolePerm, 'permission_id'):
            perm_ids = {rp.permission_id for rp in db.session.query(RolePerm.permission_id)
                                              .filter(RolePerm.role_id.in_(role_ids))}
        # If it's a Table (no ORM mapping)
        elif hasattr(RolePerm, 'c') and 'permission_id' in RolePerm.c:
            rows = db.session.execute(RolePerm.select().with_only_columns(RolePerm.c.permission_id)
                                       .where(RolePerm.c.role_id.in_(role_ids))).all()
...
def has_any_prefix(user, module: str) -> bool:
    """True si el usuario tiene algún permiso que empiece por '<module>_' o '_<module>'."""
...
def require_perm(name: str):
    """Route decorator that aborts with 403 if current user lacks the permission."""
